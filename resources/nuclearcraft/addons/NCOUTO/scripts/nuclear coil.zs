#loader nc_preinit

// || = or , && = and
mods.nuclearcraft.Registration.registerTurbineCoil("redstone", 1.20, "one copper coil && one silver coil");
mods.nuclearcraft.Registration.registerTurbineCoil("graphite", 1.04, "one beryllium coil");
mods.nuclearcraft.Registration.registerTurbineCoil("lithium", 1.10, "one connector && one graphite coil");
mods.nuclearcraft.Registration.registerTurbineCoil("lapis", 1.25, "two silver coils");
mods.nuclearcraft.Registration.registerTurbineCoil("emerald", 1.35, "one graphite coil && one lapis coil");
mods.nuclearcraft.Registration.registerTurbineCoil("diamond", 1.25, "two axial copper coils && two axial graphite coils");
mods.nuclearcraft.Registration.registerTurbineCoil("villiaumite", 1.16, "exactly two axial copper coils");
mods.nuclearcraft.Registration.registerTurbineCoil("fluorite", 1.12, "one graphite coil && one magnesium coil");
mods.nuclearcraft.Registration.registerTurbineCoil("carobbiite", 1.25, "one redstone coil && one gold coil");
mods.nuclearcraft.Registration.registerTurbineCoil("hardcarbon", 1.28, "two axial villiaumite coils");
mods.nuclearcraft.Registration.registerTurbineCoil("zircaloy", 1.33, "one hardcarbon coil");
mods.nuclearcraft.Registration.registerTurbineCoil("thermoconducting", 1.30, "one magnesium coil && one aluminum coil");
mods.nuclearcraft.Registration.registerTurbineCoil("empty",0.95, "one of any coil");
mods.nuclearcraft.Registration.registerTurbineCoil("iron",1.01, "one bearing && one magnesium coil ");
mods.nuclearcraft.Registration.registerTurbineCoil("boron",1.15, "two iron coils");
mods.nuclearcraft.Registration.registerTurbineCoil("quartz",1.40, "two axial silver coils");
mods.nuclearcraft.Registration.registerTurbineCoil("arsenic",1.15, "one iron coil || three graphite coils");
mods.nuclearcraft.Registration.registerTurbineCoil("glowstone",1.13, "one magnesium coil && two copper coils");
mods.nuclearcraft.Registration.registerTurbineCoil("nether",1.07, "two gold coils");
mods.nuclearcraft.Registration.registerTurbineCoil("obsidian",1.40, "one carobbiite coil && one silver coil ");
mods.nuclearcraft.Registration.registerTurbineCoil("nak",1.01, "two axial connector ");
mods.nuclearcraft.Registration.registerTurbineCoil("corium",0.95, "one connector ");
mods.nuclearcraft.Registration.registerTurbineCoil("end",1.13, "two connector && one magnesium coil ");
mods.nuclearcraft.Registration.registerTurbineCoil("prismarine",1.08, "one beryllium coil && one connector ");
mods.nuclearcraft.Registration.registerTurbineCoil("purpur",1.06, "one nak coil ");
mods.nuclearcraft.Registration.registerTurbineCoil("lead",1.14, "one nether coil && one silver coil ");
mods.nuclearcraft.Registration.registerTurbineCoil("manganese",1.30, "one arsenic coil && one villiaumite coil ");
mods.nuclearcraft.Registration.registerTurbineCoil("nitrogen",1.12, "one prismarine coil");
mods.nuclearcraft.Registration.registerTurbineCoil("helium",1.15, "exactly three copper coils");
mods.nuclearcraft.Registration.registerTurbineCoil("cryotheum",1.30, "four magnesium coils");
mods.nuclearcraft.Registration.registerTurbineCoil("enderium",1.09, "four connectors");
mods.nuclearcraft.Registration.registerTurbineCoil("slime",1.40, "one obsidian coil");
mods.nuclearcraft.Registration.registerTurbineCoil("tin",1.09, "two graphite coils");
mods.nuclearcraft.Registration.registerTurbineCoil("smore",1.15, "one thermoconducting coil");
mods.nuclearcraft.Registration.registerTurbineCoil("ferroboron", 1.01, "two magnesium coils");
mods.nuclearcraft.Registration.registerTurbineCoil("tough", 1.05, "one ferroboron coil");
mods.nuclearcraft.Registration.registerTurbineCoil("extreme", 1.14, "one tough coil && one graphite coil");
mods.nuclearcraft.Registration.registerTurbineCoil("mgb2", 1.18, "one extreme coil && one graphite coil");
mods.nuclearcraft.Registration.registerTurbineCoil("limno2", 1.22, "one mgb2 coil");
mods.nuclearcraft.Registration.registerTurbineCoil("hsla", 1.04, "two casings");
mods.nuclearcraft.Registration.registerTurbineCoil("sicsic", 1.03, "one of any coil && one casing");
mods.nuclearcraft.Registration.registerTurbineCoil("steel", 1.07, "one copper coil && one connector");
mods.nuclearcraft.Registration.registerTurbineCoil("bn", 1.08, "one magnesium coil && one connector");
mods.nuclearcraft.Registration.registerTurbineCoil("bas", 1.15, "one boron coil && one arsenic coil");
mods.nuclearcraft.Registration.registerTurbineCoil("rhodo", 1.15, "one helium coil && one connector");
mods.nuclearcraft.Registration.registerTurbineCoil("zirconium", 1.09, "one gold coil && one connector");




